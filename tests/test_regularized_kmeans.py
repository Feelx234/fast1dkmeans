...
from fast1dkmeans.regularized_kmeans import (
    calc_num_clusters,
    relabel_clusters,
    binary_search,
)
...
class RegularizedBasics(unittest.TestCase):
    ...
    def test_relabel_clusters(self):
        assert_array_equal(
            relabel_clusters(np.array([0, 1, 1, 3, 4, 4, 4, 4])),
            np.array([0, 1, 1, 2, 3, 3, 3, 3]),
        )
        assert_array_equal(
            relabel_clusters(np.array([0, 0, 1, 2, 3, 4, 4, 4])),
            np.array([0, 0, 1, 1, 2, 2, 2, 2]),
        )
        assert_array_equal(
            relabel_clusters(np.array([0, 0, 0, 0, 0])), np.array([0, 0, 0, 0, 0])
        )
    ...
    def test_calc_cluster_cost_implicit(self):
        ...
        self.assertEqual(
            calc_cluster_cost_implicit(np.array([0, 0, 2, 2]), cc.cumsum, cc.cumsum2),
            1.0,
        )
        ...
        self.assertEqual(
            calc_cluster_cost_implicit(
                np.array([0, 0, 2, 2, 4, 4, 4]), cc.cumsum, cc.cumsum2
            ),
            3.0,
        )
        ...
        for k, n in self.kn:
            for seed in range(self.n_sampels):
                ...
                for method in [0, 1]:
                    ...
                    np.testing.assert_array_equal(
                        clusters, result, f"seed={seed} k={k} n={n} method={method}"
                    )
