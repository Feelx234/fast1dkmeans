...
from fast1dkmeans.common import (
    calc_cumsum,
    calc_cumsum2,
    calc_objective,
    CumsumCalculator,
...
class WilberCalculator:
    ...
    def calc(self, j, i):  # i <-> j interchanged is not a bug!
        ...
        return (
            calc_objective(self.cumsum, self.cumsum2, i, j)
            + self.lambda_
            + self.F_vals[i]
        )
...
def binary_search(v, k, max_iter=200, epsilon=1e-10, method=0):
    ...
    for _ in range(max_iter):
        ...
        if k_mid == k:
            ...
            k_high = k_mid
            if method == 1:
                c_high = calc_cluster_cost_implicit(
                    result, calculator.cumsum, calculator.cumsum2
                )
            ...
            k_low = k_mid
            if method == 1:
                c_low = calc_cluster_cost_implicit(
                    result, calculator.cumsum, calculator.cumsum2
                )
